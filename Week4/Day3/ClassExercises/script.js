...
const newObj2 = famousPeople.map((person) => ({
  name: person["name"],
  Job: person["job"],
}));
...
const newCharacters = characters.filter((char) =>
  char["series"].includes("Star Trek: The Next Generation")
);
...
const sumOfScore = students.reduce((acc, person) => acc + person["score"], 0);
console.log(sumOfScore);

// 1loop
// acc = 0
// person = {name: 'Rich', score: 33},
// return acc+person["score"] - 0+33 - 33

// // 2loop
// acc = 33
// person = {name: 'Peter', score: 55},
// return acc+person["score"] - 33+55 - 88
...
// because true is equal to 1 and false is equal to 0
const numVotersSecond = voters.reduce(
  (acc, person) => acc + person["voted"],
  0
);
...
// //1 loop
// acc = 0
// person {name:'Bob' , age: 30, voted: true},
// person["voted"] is true
// return 0+1

// // 3loop
// acc = 2
// person {name:'Kate' , age: 25, voted: false},
// person["voted"] false
// return acc+0
